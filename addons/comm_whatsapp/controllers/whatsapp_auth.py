...
class WhatsAppAuthController(http.Controller):
    ...
    def oauth_callback(self, code=None, state=None, error=None, error_description=None):
        ...
        try:
            ...
            app_secret = IrConfigParameter.get_param('comm_whatsapp.app_secret')
            redirect_uri = IrConfigParameter.get_param('comm_whatsapp.redirect_uri', 
            ...
            IrConfigParameter.set_param('comm_whatsapp.access_token', access_token)
            
            # Optionally get long-lived token
            long_lived_token = self._get_long_lived_token(access_token, app_id, app_secret)
            if long_lived_token:
                IrConfigParameter.set_param('comm_whatsapp.long_lived_token', long_lived_token)
                IrConfigParameter.set_param('comm_whatsapp.access_token', long_lived_token)

            _logger.info("WhatsApp authentication successful")
            return request.render('comm_whatsapp.oauth_success', {
    ...
    def initiate_auth(self):
        ...
        try:
            ...
            if not redirect_uri:
                ...
                IrConfigParameter.set_param('comm_whatsapp.redirect_uri', redirect_uri)
            scope = IrConfigParameter.get_param('comm_whatsapp.scope')
            if not scope:
                scope = 'whatsapp_business_management,whatsapp_business_messaging'
                IrConfigParameter.set_param('comm_whatsapp.scope', scope)
