...
class ResConfigSettings(models.TransientModel):
    ...
    # OAuth / Meta App
    whatsapp_light_app_id = fields.Char(
        string='Meta App ID',
        config_parameter='whatsapp_light.app_id',
        help='Facebook/Meta App ID from developers.facebook.com',
    )
    whatsapp_light_app_secret = fields.Char(
        string='Meta App Secret',
        config_parameter='whatsapp_light.app_secret',
        help='Facebook/Meta App Secret',
    )
    whatsapp_light_redirect_uri = fields.Char(
        string='OAuth Redirect URI',
        config_parameter='whatsapp_light.redirect_uri',
        help='Must match the URI configured in Meta App. Default: {base_url}whatsapp/auth/callback',
    )
    whatsapp_light_scope = fields.Char(
        string='OAuth Scope',
        config_parameter='whatsapp_light.scope',
    ...
    # Webhook
    whatsapp_light_webhook_verify_token = fields.Char(
        string='Webhook Verify Token',
        config_parameter='whatsapp_light.webhook_verify_token',
    ...
    whatsapp_light_access_token = fields.Char(
        string='Access Token',
        config_parameter='whatsapp_light.access_token',
        help='Current access token. Usually set automatically after OAuth; edit only if needed.',
    )
    whatsapp_light_long_lived_token = fields.Char(
        string='Long-Lived Token',
        config_parameter='whatsapp_light.long_lived_token',
        help='Long-lived token (60 days). Set automatically after OAuth.',
    )

    # From webhook or set manually
    whatsapp_light_business_account_id = fields.Char(
        string='Business Account ID',
        config_parameter='whatsapp_light.business_account_id',
        help='WhatsApp Business Account ID. Can be set automatically from the first webhook.',
    )
    whatsapp_light_phone_number_id = fields.Char(
        string='Phone Number ID',
        config_parameter='whatsapp_light.phone_number_id',
