...
class WhatsAppTemplateSendWizard(models.TransientModel):
    ...
    def action_send_template(self):
        ...
        try:
            ...
            # According to WhatsApp Flows API docs: https://developers.facebook.com/docs/whatsapp/flows/guides/sendingaflow/
            flow_button_index = 0
            for button in self.template_id.button_ids:
                if button.button_type == 'FLOW':
                    if not button.flow_id:
                        _logger.warning(f"Skipping FLOW button '{button.text}' - no flow configured")
                        continue
                    
                    if button.flow_id.status != 'PUBLISHED':
                        return {
                            'type': 'ir.actions.client',
                            'tag': 'display_notification',
                            'params': {
                                'title': 'Error',
                                'message': f'Flow "{button.flow_id.name}" must be published before use. Current status: {button.flow_id.status}',
                                'type': 'danger',
                                'sticky': True,
                            }
                        }
                    ...
                    elif button.flow_id.first_page_id:
                        flow_action_data['screen'] = button.flow_id.first_page_id
