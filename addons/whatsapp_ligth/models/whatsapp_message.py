...
class WhatsAppMessage(models.Model):
    ...
    # Chatbot fields (if chatbot module is installed)
    # Note: These fields will only work if whatsapp_ligth_chatbot module is installed
    chatbot_message_ids = fields.One2many('whatsapp.chatbot.message', 'wa_message_id', 
                                         string='Chatbot Messages', readonly=True,
                                         help='Related chatbot messages (requires whatsapp_ligth_chatbot module)')
    chatbot_id = fields.Many2one('whatsapp.chatbot', string='Chatbot', 
                                 compute='_compute_chatbot_info', store=True, readonly=True,
                                 help='Chatbot associated with this message (requires whatsapp_ligth_chatbot module)')
    chatbot_name = fields.Char(string='Chatbot Name', 
                               compute='_compute_chatbot_info', store=True, readonly=True,
                               help='Name of the chatbot associated with this message (requires whatsapp_ligth_chatbot module)')
        ...
        for record in self:
            ...
            if 'whatsapp.chatbot.message' in self.env:
                ...
                if chatbot_message and chatbot_message.chatbot_id:
                    ...
                    if phone_to_check and 'whatsapp.chatbot.contact' in self.env:
                        ...
                        if partner:
                            ...
                            if chatbot_contact and chatbot_contact.last_chatbot_id:
                                ...
                                # Also check if message timestamp is close to last interaction (within reasonable time)
