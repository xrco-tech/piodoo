...
class WhatsappFreeWindow(models.Model):
    _name = 'whatsapp.free.window'
    _description = 'WhatsApp free-messaging window (24h CS or 72h entry point)'
    ...
    source_message_id = fields.Many2one('whatsapp.message',
        help='Inbound message that opened this window (for CS-24h).')
    ...
    @api.model
    def open_window(self, account, wa_id, partner=None, window_type='cs_24h',
                    opened_at=None, source_message=None):
        """Open or extend a free window. Idempotent: extends the existing open
    ...
    @api.model
    def covers(self, account, wa_id, at_datetime, window_type='cs_24h'):
