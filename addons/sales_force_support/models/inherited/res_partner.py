...
# ─────────────────────────────────────────────────────────────────────────────
# ConsumerView resolve wizard extension  (source: botle_buhle_custom)
# ─────────────────────────────────────────────────────────────────────────────


class PartnerConsumerViewResolve(models.TransientModel):
    _inherit = "partner.consumerview.resolve"
    ...
    def action_choose(self):
        ...
        self.partner_id.write(
            {
                "name": (
                    self.chosen_line_id.first_name
                    + " "
                    + self.chosen_line_id.last_name
                ),
                "first_name": self.chosen_line_id.first_name,
                "unverified_first_name": self.chosen_line_id.first_name,
                "last_name": self.chosen_line_id.last_name,
                "unverified_last_name": self.chosen_line_id.last_name,
                "unverified_street": self.chosen_line_id.street,
                "unverified_suburb": self.chosen_line_id.suburb,
                "unverified_city": self.chosen_line_id.city,
                "unverified_state_id": self.chosen_line_id.state_id.id,
                "unverified_country_id": self.chosen_line_id.state_id.country_id.id,
                "unverified_zip": self.chosen_line_id.zip,
                "consumerview_ref": self.ref,
            }
        )

        body = "<p>Data populated from ConsumerView:</p>\n<ul>\n"
        body += f"\t<li><strong>Name:</strong> {self.chosen_line_id.name}</li>\n"
        body += f"\t<li><strong>Street:</strong> {self.chosen_line_id.street}</li>\n"
        body += f"\t<li><strong>Suburb:</strong> {self.chosen_line_id.suburb}</li>\n"
        body += f"\t<li><strong>City:</strong> {self.chosen_line_id.city}</li>\n"
        body += f"\t<li><strong>Postcode:</strong> {self.chosen_line_id.zip}</li>\n"
        body += (
            f"\t<li><strong>Province:</strong> "
            f"{self.chosen_line_id.state_id.name}</li>\n"
        )
        body += (
            f"\t<li><strong>Country:</strong> "
            f"{self.chosen_line_id.state_id.country_id.name}</li>\n"
        )
        body += (
            f"\t<li><strong>ConsumerView Reference:</strong> {self.ref}</li>\n"
        )
        body += "</ul>"
        if len(self.line_ids) > 1:
            body += (
                f"\n<em>There were {len(self.line_ids) - 1} other result(s) "
                f"returned from ConsumerView.</em>"
            )
        if self.env.company.consumerview_env != "prod":
            body += (
                "\n<em>This request was executed with the sandbox environment.</em>"
            )

        self.partner_id.message_post(body=body, body_is_html=True)
